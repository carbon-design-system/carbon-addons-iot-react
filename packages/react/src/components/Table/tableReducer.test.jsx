...
describe('table reducer', () => {
  ...
  it('row action tests', () => {
    const updatedRowActionState = tableReducer(
      initialState,
      tableRowActionStart('row-1')
    );
    ...
    // Clear the action
    const updatedRowActionState3 = tableReducer(
      initialState,
      tableRowActionComplete('row-1')
    );
    ...
    const updatedRowActionState4 = tableReducer(
      initialState,
      tableRowActionEdit('row-1')
    );
  ...
  describe('filter tests', () => {
    it('TABLE_FILTER_CLEAR', () => {
      expect(
        tableReducer(initialState, tableFilterClear()).view.filters
      ).toEqual([]);
    });
    it('TABLE_FILTER_APPLY filter should filter data', () => {
      ...
      expect(updatedState.view.filters).toEqual([
        { columnId: tableColumns[0].id, value: '1' },
      ]);
      // data should result in less records
      expect(updatedState.view.table.filteredData.length).toBeLessThan(
  ...
  describe('pagination', () => {
    ...
    it('TABLE_PAGE_CHANGE with invalid page', () => {
      const updatedState = tableReducer(
        initialState,
        tablePageChange({ page: 65, pageSize: 10 })
      );
  ...
  describe('toolbar actions', () => {
    ...
    it('TABLE_SEARCH_APPLY filter should search data', () => {
      ...
      const updatedState = tableReducer(
        initialState,
        tableSearchApply(searchString)
      );
      // Apply the search
      expect(updatedState.view.toolbar.search.defaultValue).toEqual(
        searchString
      );
      expect(updatedState.view.pagination.page).toEqual(1);
    });
  ...
  describe('table actions', () => {
    it('TABLE_ACTION_CANCEL ', () => {
      const tableWithSelection = tableReducer(
        initialState,
        tableRowSelectAll(true)
      );
      // All should be selected
      expect(tableWithSelection.view.table.selectedIds.length).toEqual(
        initialState.data.length
      );
      ...
      expect(tableAfterCancel.view.table.isSelectAllIndeterminate).toEqual(
        false
      );
    });
    it('TABLE_ACTION_APPLY ', () => {
      ...
      const tableAfterCustomAction = tableReducer(
        tableWithSelection,
        tableActionApply('custom')
      );
      expect(tableAfterCustomAction.view.table.selectedIds).toEqual([]);
      expect(tableAfterCustomAction.view.table.isSelectAllSelected).toEqual(
        false
      );
      expect(
        tableAfterCustomAction.view.table.isSelectAllIndeterminate
      ).toEqual(false);
      ...
      const tableAfterDeleteAction = tableReducer(
        tableWithSelection,
        tableActionApply('delete')
      );
      expect(tableAfterDeleteAction.view.table.selectedIds).toEqual([]);
      expect(tableAfterDeleteAction.view.table.isSelectAllSelected).toEqual(
        false
      );
      expect(
        tableAfterDeleteAction.view.table.isSelectAllIndeterminate
      ).toEqual(false);
  ...
  describe('table column actions', () => {
    it('TABLE_COLUMN_SORT', () => {
      ...
      const filteredTable = tableReducer(
        initialState,
        tableRegister({ data: initialState.data })
      );
    ...
    it('TABLE_COLUMN_ORDER', () => {
      ...
      const columnHideAction = tableColumnOrder([
        { columnId: tableColumns[0].id, isHidden: true },
      ]);
      const tableWithHiddenColumn = tableReducer(
        initialState,
        columnHideAction
      );
  ...
  describe('Table Row Operations', () => {
    it('TABLE_ROW_SELECT', () => {
      ...
      expect(tableWithNoRowsSelected.view.table.isSelectAllSelected).toEqual(
        false
      );
      expect(
        tableWithNoRowsSelected.view.table.isSelectAllIndeterminate
      ).toEqual(false);
      ...
      expect(tableWithSelectedRow.view.table.isSelectAllSelected).toEqual(
        false
      );
      expect(tableWithSelectedRow.view.table.isSelectAllIndeterminate).toEqual(
        true
      );
      ...
      expect(tableWithUnSelectedRow.view.table.isSelectAllSelected).toEqual(
        false
      );
      expect(
        tableWithUnSelectedRow.view.table.isSelectAllIndeterminate
      ).toEqual(false);
    });
    it('TABLE_ROW_SELECT--SINGLE', () => {
      ...
      expect(tableWithPreviouslySelectedRow.view.table.selectedIds).toEqual([
        'row-2',
      ]);
      expect(
        tableWithPreviouslySelectedRow.view.table.isSelectAllSelected
      ).toEqual(false);
      expect(
        tableWithPreviouslySelectedRow.view.table.isSelectAllIndeterminate
      ).toEqual(true);
    });
    it('TABLE_ROW_SELECT_ALL', () => {
      expect(initialState.view.table.selectedIds).toEqual([]);
      // Select all
      const tableWithSelectedAll = tableReducer(
        initialState,
        tableRowSelectAll(true)
      );
      ...
      expect(tableWithSelectedAll.view.table.isSelectAllIndeterminate).toEqual(
        false
      );
      expect(tableWithSelectedAll.view.table.isSelectAllSelected).toEqual(true);
      // Unselect all
      const tableWithUnSelectedAll = tableReducer(
        initialState,
        tableRowSelectAll(false)
      );
      expect(tableWithUnSelectedAll.view.table.selectedIds.length).toEqual(0);
      expect(
        tableWithUnSelectedAll.view.table.isSelectAllIndeterminate
      ).toEqual(false);
      expect(tableWithUnSelectedAll.view.table.isSelectAllSelected).toEqual(
        false
      );
    });
    it('TABLE_ROW_EXPAND', () => {
      ...
      // Expanded row
      const tableWithExpandedRow = tableReducer(
        initialState,
        tableRowExpand('row-1', true)
      );
      expect(tableWithExpandedRow.view.table.expandedIds).toEqual(['row-1']);
      // Collapsed row
      const tableWithCollapsedRow = tableReducer(
        initialState,
        tableRowExpand('row-1', false)
      );
    ...
    it('REGISTER_TABLE', () => {
      ...
      expect(
        tableWithFilteredData.view.table.filteredData.length
      ).toBeGreaterThan(0);
      expect(tableWithFilteredData.view.table.isSelectAllSelected).toEqual(
        false
      );
      expect(tableWithFilteredData.view.table.isSelectAllIndeterminate).toEqual(
        false
      );
      expect(tableWithFilteredData.view.table.filteredData.length).not.toEqual(
        initialState.data.length
      );
      expect(tableWithFilteredData.view.table.loadingState.isLoading).toEqual(
        false
      );

      // Initial state with sort specified but no filters
      const initialStateWithSortAndNoFilters = merge(
        {},
        omit(initialState, 'view.filters'),
        {
          view: {
...
describe('filter, search and sort', () => {
  it('filterData', () => {
    const mockData = [
      { values: { number: 10, node: <Add20 />, string: 'string', null: null } },
    ];
    ...
    expect(
      filterData(mockDataWithMultiselect, [multiselectFilter])
    ).toHaveLength(2);
  });
  ...
  it('filterData with custom comparison', () => {
    ...
    const customFilterFunction = (columnFilterValue, value) =>
  ...
  it('filterSearchAndSort', () => {
    ...
    expect(
      filterSearchAndSort(mockData, {}, {}, [
        { columnId: 'string', value: 'string' },
      ])
    ).toHaveLength(1);
    expect(
      filterSearchAndSort(mockData, {}, {}, [
        { columnId: 'string', value: 'none' },
      ])
    ).toHaveLength(0);
  });
