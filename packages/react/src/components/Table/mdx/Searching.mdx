...
Two actions are triggered when a user opens search input field. First, the callback `onSearchExpand` will be called upon opening/closing. This callback has 2 arguments: event and expand state. Second, boolean parameter `isExpanded` indicating current search input state.
