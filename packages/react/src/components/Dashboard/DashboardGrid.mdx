...
grid so that no cards overlap. The height (h) and width (w) are unused, because sizes are determined
by the size prop passed to the card itself—not defined by the grid.
...
only triggered once you've stopped resizing the card.
