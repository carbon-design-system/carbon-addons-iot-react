...
describe('Card', () => {
  ...
  it('render custom icons', () => {
    ...
    const { container } = render(
      <Tree16 aria-hidden aria-label="Expand to fullscreen" />
    );
  ...
  it('additional prop based elements', () => {
    ...
    expect(
      wrapper.find(`.${iotPrefix}--card--header`).find(Tooltip)
    ).toHaveLength(1);
  ...
  it('card actions', () => {
    ...
    expect(mockOnCardAction).toHaveBeenCalledWith(
      cardProps.id,
      CARD_ACTIONS.CLOSE_EXPANDED_CARD
    );
    ...
    expect(mockOnCardAction).toHaveBeenCalledWith(
      cardProps.id,
      CARD_ACTIONS.OPEN_EXPANDED_CARD
    );
  });
  ...
  it('card toolbar renders in header only when there are actions', () => {
    ...
    expect(
      wrapperWithActions
        .getDOMNode()
        .querySelectorAll(
          `.${iotPrefix}--card--header .${iotPrefix}--card--toolbar`
        )
    ).toHaveLength(1);

    const wrapperWithoutActions = mount(
      <Card {...cardProps} isExpanded size={CARD_SIZES.SMALL} />
    );
    expect(
