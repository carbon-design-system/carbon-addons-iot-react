...
export const baseTableReducer = (state = {}, action) => {
  ...
  if (
    ...
    action.instanceId !== state.instanceId
  ) {
    return state;
  }
  ...
  switch (action.type) {
    ...
    case TABLE_TOOLBAR_TOGGLE: {
      const filterToggled =
        state.view.toolbar.activeBar === action.payload ? null : action.payload;
      return update(state, {
        view: {
          toolbar: {
            activeBar: {
      ...
      return update(state, {
        view: {
          table: {
            ...
            rowActions: {
              $splice: [
                [index, 1, { rowId: action.payload, error: action.error }],
              ],
            },
          },
        },
    ...
    case TABLE_ROW_SELECT: {
      ...
      const isClearing =
        ...
        !isSelected &&
        state.view.table.selectedIds.length <= 1;
